...
class ImageGradientsLayer(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # Sobel filter
        self.conv2d.weight = get_sobel_xy_parameters(self.conv2d)
...
def get_gaussian_kernel(ksize=7, sigma=5) -> torch.nn.Parameter:
    ...
    important that you get the correct value here in order to pass the unit tests for HarrisNet.fsecondmm
    ...
    #raise NotImplementedError('`get_gaussian_kernel` need to be ' + 'implemented')
...
def get_sobel_xy_parameters(kernel) -> torch.nn.Parameter:
    """
    ...
    #kernel= nn.Conv2d(1, 2, kernel_size=3, stride=1, padding=1)
    ...
    #raise NotImplementedError('`get_sobel_xy_parameters` need to be '+ 'implemented')
