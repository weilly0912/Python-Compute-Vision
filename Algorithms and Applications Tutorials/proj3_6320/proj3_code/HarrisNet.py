...
class HarrisNet(nn.Module):
    ...
    def __init__(self):
        ...
        # TODO: YOUR CODE HERE                                                #
        #######################################################################
        #raise NotImplementedError('`HarrisNet.__init__` function in '+ '`HarrisNet.py` needs to be implemented')
        self.net = None # <--replace this with your implementation
        self.ImageGradientsLayer = ImageGradientsLayer()
        self.ChannelProductLayer = ChannelProductLayer()
        self.SecondMomentMatrixLayer = SecondMomentMatrixLayer(ksize=3, sigma=3)
        self.CornerResponseLayer = CornerResponseLayer(alpha=0.05)
        self.NMSLayer = NMSLayer()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        x = self.ImageGradientsLayer(x)
        x = self.ChannelProductLayer(x)
        x = self.SecondMomentMatrixLayer(x)
        R = self.CornerResponseLayer(x)
        Rnms = self.NMSLayer(R) 
...
class ChannelProductLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #raise NotImplementedError('`ChannelProductLayer` need to be '+ 'implemented')
        B,C,W,H = list(x.size()) # size([1,2,W,H])
        Ix2 = x[:,0,:,:]**2
        Iy2 = x[:,1,:,:]**2
        Ixy = x[:,0,:,:]*x[:,1,:,:]
...
class SecondMomentMatrixLayer(torch.nn.Module):
    ...
    def __init__(self, ksize: torch.Tensor = 7, sigma: torch.Tensor = 5):
        ...
        #raise NotImplementedError('`SecondMomentMatrixLayer` need to be '+ 'implemented')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #using conv2d to create gaussian kernel
        gaussian_kernel= torch.nn.Conv2d(1, 1, kernel_size=self.ksize, stride=1, padding=1)
        gaussian_kernel.weight.data[0, 0].copy_(self.kernel)
        gaussian_kernel.bias.data.zero_()

        if C==3 :
            x1, x2, x3  = torch.split(x, 1, dim = 1)

            #calcuate new gaussian data
...
class CornerResponseLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        c = x[:,1,:,:]
        d = x[:,2,:,:]
        det_matrix   = torch.mul(a,c)-torch.mul(b,d)
        trace_matrix = a+c
        output = det_matrix - torch.mul(self.alpha ,torch.mul(trace_matrix,trace_matrix))
        output = output.view(1,W,H).unsqueeze(0)
...
class NMSLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        #https://blog.csdn.net/u011534057/article/details/77775467
        #######################################################################
        # TODO: YOUR CODE HERE                                                #
        #######################################################################
