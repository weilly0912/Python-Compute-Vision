...
class SubGridAccumulationLayer(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        #raise NotImplementedError('`__init__` in `SubGridAccumulationLayer` ' + 'needs to be implemented')
        self.layer = None
        self.layer = torch.nn.Conv2d(8, 8, 4, padding=(2, 2), bias=None, groups=8)
        w = torch.ones(8, 1, 4, 4)
        self.layer.weight = torch.nn.Parameter(w)
...
def angles_to_vectors_2d_pytorch(angles: torch.Tensor) -> torch.Tensor:
    ...
    angle_vectors = torch.stack((torch.cos(angles),torch.sin(angles)), 1)
...
class SIFTOrientationLayer(nn.Module):
    ...
    def get_orientation_bin_weights(self) -> torch.nn.Parameter():
        ...
        #raise NotImplementedError('`get_orientation_bin_weights` needs to be ' + 'implemented')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        #return self.layer(x)
        return torch.cat((self.layer(x), x), 1)
...
def get_sift_subgrid_coords(x_center: int, y_center: int):
    ...
    #raise NotImplementedError('`get_sift_subgrid_coords` needs to be '+ 'implemented')
...
def get_siftnet_features(img_bw: torch.Tensor, x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    for i in range(x.shape[0]):
        x_grid, y_grid = get_sift_subgrid_coords(x[i], y[i])#16,16
        for j in range(x_grid.shape[0]):
            for k in range(sift.shape[1]):
                vector[i][(j * sift.shape[1]) + k] = sift[0][k][y_grid[j]][x_grid[j]]
        vector[i] = vector[i] / np.sqrt(np.sum(np.multiply(vector[i], vector[i]))) # Norm
    fvs = np.power(vector, 0.9)
